...
// --- НОВЫЙ КОД: КОНСТАНТЫ ДЛЯ ТОНА КОЖИ ---
const skinToneModifiers = ['🏻', '🏼', '🏽', '🏾', '🏿'];
const emojiWithSkinToneSupport = new Set('👋🤚🖐✋🖖👌🤏✌🤞🤟🤘🤙👈👉👆🖕👇☝👍👎✊👊🤛🤜👏🙌👐🤲🙏✍💅🤳💪🦵🦶👂👃👶🧒👦👧🧑👱👨🧔👩🧓👴👵👮👷💂🕵'.split(''));
// --- КОНЕЦ НОВОГО КОДА ---
...
export function getHtml() {
    return `
        <style>
            ...
            /* НОВЫЕ СТИЛИ ДЛЯ ВСПЛЫВАЮЩЕГО МЕНЮ */
            #skin-tone-popup {
                display: none;
                position: absolute;
                z-index: 50;
        ...
        <div class="space-y-4">
            ...
            <!-- НОВЫЙ ЭЛЕМЕНТ: КОНТЕЙНЕР ДЛЯ ВСПЛЫВАЮЩЕГО МЕНЮ -->
...
function createItemButton(symbol, isEmoji) {
    ...
    if(t) btn.title = t;
    // --- ИЗМЕНЕНИЕ: ДОБАВЛЕН АТРИБУТ ДЛЯ ПОДДЕРЖКИ ТОНА КОЖИ ---
    if (isEmoji && emojiWithSkinToneSupport.has(s)) {
        btn.dataset.skinToneSupport = 'true';
    }
    return btn;
}
...
export async function init() {
    ...
    // --- НОВЫЙ КОД: ПОЛУЧЕНИЕ ЭЛЕМЕНТА POPUP ---
    ...
    // --- ИЗМЕНЕНИЕ: ОБНОВЛЕННЫЙ ОБРАБОТЧИК КЛИКОВ ---
    document.getElementById('app-content-container').addEventListener('click', (e) => {
        const button = e.target.closest('button[data-copy]');
        if (!button) return;

        // Если клик был внутри всплывающего меню, ничего не делаем
        if (skinTonePopup.contains(button)) return;

        // Если эмодзи поддерживает тон кожи, показываем меню
        if (button.dataset.skinToneSupport === 'true') {
            e.stopPropagation(); // Предотвращаем закрытие меню сразу после открытия
            skinTonePopup.innerHTML = ''; // Очищаем предыдущие опции

            skinToneModifiers.forEach(modifier => {
                const option = document.createElement('button');
                option.className = 'skin-tone-option';
                const modifiedEmoji = button.dataset.copy + modifier;
                option.textContent = modifiedEmoji;
                option.onclick = () => {
                    navigator.clipboard.writeText(modifiedEmoji);
                    const dataToSave = { s: modifiedEmoji, c: modifiedEmoji, k: button.dataset.keywords };
                ...
                skinTonePopup.appendChild(option);
            });
    ...
    // --- НОВЫЙ КОД: ЗАКРЫТИЕ МЕНЮ ПРИ КЛИКЕ ВНЕ ЕГО ---
    document.addEventListener('click', (e) => {
        if (!skinTonePopup.contains(e.target) && !e.target.closest('[data-skin-tone-support="true"]')) {
            skinTonePopup.style.display = 'none';
        }
    });
