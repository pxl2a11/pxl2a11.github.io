...
export function getHtml() {
    return `
        <style>
            ...
            /* НОВЫЕ СТИЛИ ДЛЯ ВЫБОРА ТОНА КОЖИ */
            #skin-tone-popup {
                display: none; position: absolute; z-index: 60;
                background-color: white; border-radius: 0.5rem; padding: 0.5rem;
                box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -2px rgba(0,0,0,0.1);
                border: 1px solid #e5e7eb;
            }
            .dark #skin-tone-popup { background-color: #374151; border-color: #4b5563; }
            .skin-tone-btn { font-size: 1.5rem; padding: 0.25rem; border-radius: 0.25rem; }
            .skin-tone-btn:hover { background-color: #e5e7eb; }
            .dark .skin-tone-btn:hover { background-color: #4b5563; }
        </style>
        <div class="space-y-4">
            ...
            <!-- НОВЫЙ ЭЛЕМЕНТ: POPUP ДЛЯ ТОНА КОЖИ -->
...
function createItemButton(symbol, isEmoji) {
    ...
    if(t) btn.title = t;
    // --- ИЗМЕНЕНИЕ: ДОБАВЛЯЕМ ДАННЫЕ ДЛЯ ТОНА КОЖИ ---
    if (isEmoji && EMOJI_WITH_SKIN_TONES.has(s)) {
        btn.dataset.skintone = "true";
    }
    return btn;
}
//... (остальные функции без изменений до init)
...
export async function init() {
    ...
    const skinTonePopup = document.getElementById('skin-tone-popup'); // Новый элемент
    ...
    // --- НОВАЯ ФУНКЦИЯ: ПОКАЗ ВЫБОРА ТОНА КОЖИ ---
    const showSkinTonePopup = (button) => {
        ...
        SKIN_TONE_MODIFIERS.forEach(modifier => {
            ...
            toneBtn.onclick = () => {
                navigator.clipboard.writeText(toneBtn.textContent);
                addToRecent(RECENT_EMOJI_KEY, { s: toneBtn.textContent, c: toneBtn.textContent });
                showNotification();
        ...
        skinTonePopup.style.left = `${rect.left}px`;
        skinTonePopup.style.top = `${rect.bottom + 5}px`;
    };

    const updateRecentAndRender = async (tab) => { /* ... (без изменений) ... */ const key = tab === 'symbols' ? RECENT_SYMBOLS_KEY : RECENT_EMOJI_KEY; const recentItems = await getRecent(key); if (dataMap[tab] && dataMap[tab][0] && dataMap[tab][0].name === 'Недавно использованные') { dataMap[tab][0].items = recentItems; } const data = dataMap[tab]; renderContent(contentArea, data, tab === 'emojis'); renderNav(navArea, data); setupIntersectionObserver(); };
    const handleSearch = () => { /* ... (без изменений) ... */ };
    
    tabs.forEach(tab => { tab.addEventListener('click', () => { /* ... (без изменений) ... */ }); });
    searchInput.addEventListener('input', handleSearch);
    navArea.addEventListener('click', e => { /* ... (без изменений) ... */ });

    // --- ОБНОВЛЕННЫЙ ОБРАБОТЧИК КЛИКОВ ---
    document.getElementById('app-content-container').addEventListener('mousedown', e => {
        const button = e.target.closest('button[data-copy]');
        if (button && button.dataset.skintone) {
    ...
    document.getElementById('app-content-container').addEventListener('mouseup', e => {
        if(longPressTimer) {
            ...
            if (button) {
                ...
                addToRecent(isEmoji ? RECENT_EMOJI_KEY : RECENT_SYMBOLS_KEY, { s: copyText, c: copyText });
     ...
     document.getElementById('app-content-container').addEventListener('contextmenu', e => {
        const button = e.target.closest('button[data-copy]');
        if (button && button.dataset.skintone) {
            e.preventDefault();
            showSkinTonePopup(button);
    ...
    function setupIntersectionObserver() { /* ... (без изменений) ... */ }
