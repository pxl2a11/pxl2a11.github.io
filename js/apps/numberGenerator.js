...
export function init() {
    ...
    generateBtn.addEventListener('click', () => {
        ...
        if (min >= max) { 
            errorEl.textContent = 'Минимальное значение должно быть меньше максимального.'; 
            return; 
        }
        if (count < 1 || count > 50) {
            errorEl.textContent = 'Количество должно быть от 1 до 50.'; 
            return;
        }
        ...
        setTimeout(() => {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if(count === 1) {
                    resultEl.style.fontSize = '4.5rem'; // 72px
                    resultEl.textContent = randomNumber;
                } else {
                     resultEl.style.fontSize = '2.25rem'; // 36px
                    const numberSpan = document.createElement('span');
                    numberSpan.className = 'bg-gray-200 dark:bg-gray-700 rounded-lg p-2';
