...
export function init() {
    // <<< ИЗМЕНЕНИЕ ЗДЕСЬ: Добавляем класс к body при инициализации
    ...
    function createStationButtons() {
        ...
        radioStations.forEach((station, index) => {
            ...
            logoContainer.className = 'w-20 h-20 rounded-full flex items-center justify-center overflow-hidden border-2 border-transparent group-hover:border-blue-500 transition-colors duration-200';
            ...
            if (station.logoUrl) {
                ...
                img.onerror = () => {
                    ...
                    fallbackIcon.className = 'w-full h-full flex items-center justify-center text-xs bg-gray-300 dark:bg-gray-600 text-gray-600 dark:text-gray-300';
            ...
            stationNameContainer.className = "h-12 flex justify-center items-center w-full mt-2";
    ...
    function selectStation(station, buttonElement) {
        document.querySelectorAll('#radio-stations .station-card').forEach(btn => btn.classList.remove('ring-2', 'ring-blue-500', 'scale-105'));
        if (buttonElement) {
            buttonElement.classList.add('ring-2', 'ring-blue-500', 'scale-105');
        }
        ...
        // Устанавливаем отступ, чтобы плеер не перекрывал контент
    ...
    playPauseBtn.disabled = currentStation === null;
}

export function cleanup() {
    // <<< ИЗМЕНЕНИЕ ЗДЕСЬ: Убираем класс с body при выходе из приложения
