...
export function getHtml() {
    return `
        <!-- Корневой контейнер для позиционирования плеера -->
        <div class="relative w-full h-full flex flex-col">
            
            <!-- Этот блок будет прокручиваться, занимая все доступное место -->
            <div id="radio-list-wrapper" class="flex-grow overflow-y-auto p-4">
                <div class="mb-4">
            ...
            <!-- Плеер позиционируется абсолютно относительно корневого контейнера -->
            <div id="fixed-player-container" class="absolute bottom-0 left-0 right-0 z-10 p-4 bg-white/70 dark:bg-gray-900/70 backdrop-blur-lg border-t border-gray-200 dark:border-gray-700 shadow-xl hidden">
...
export function init(appContentContainer) {
    // Устанавливаем стили для родительского контейнера, чтобы наша верстка работала
    appContentContainer.style.height = '100%';
    ...
    const listWrapper = appContentContainer.querySelector('#radio-list-wrapper');
    ...
    function selectStation(station, buttonElement) {
        ...
        // Устанавливаем padding-bottom для прокручиваемого списка
        if (listWrapper) {
            listWrapper.style.paddingBottom = `${fixedPlayerContainer.offsetHeight}px`;
        }
...
export function cleanup(appContentContainer) {
    // Убираем стили с родительского контейнера
    appContentContainer.style.height = '';
