...
export function getHtml() {
    ...
    return `
        <style>
            .keyboard-layout { padding: 0.5rem; background-color: #212529; border-radius: 0.5rem; box-shadow: 0 8px 16px rgba(0,0,0,0.3), inset 0 -4px 4px rgba(0,0,0,0.2); display: flex; flex-direction: column; gap: 0.25rem; width: 100%; overflow-x: auto; }
            .keyboard-inner-wrapper { display: flex; flex-direction: column; gap: 0.25rem; min-width: 980px; } /* Установлена мин. ширина */
            .key-row { display: flex; gap: 0.25rem; }
            .key { position: relative; display: flex; align-items: center; justify-content: center; flex: 1 1 0px; min-width: 42px; height: 42px; padding: 4px; background: linear-gradient(to top, #495057, #6c757d); border: 1px solid #343a40; border-radius: 0.25rem; box-shadow: inset 0 -2px 0 rgba(0,0,0,0.2); color: #f8f9fa; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; font-size: 11px; font-weight: 600; text-align: center; transition: all 0.05s ease-out; user-select: none; }
            ...
            .key--h-2 { height: 88px; } /* (42px * 2) + 4px gap */
        ...
        <div class="flex flex-col gap-6 items-center">
            ...
            <div id="virtual-keyboard" class="keyboard-layout w-full max-w-7xl mx-auto">
                <div class="keyboard-inner-wrapper">
                    <!-- ИЗМЕНЕНО: Верхний ряд с точным позиционированием -->
                    <div class="key-row">
                        ${key('Esc', 'Escape')} <div class="key-placeholder" style="flex-grow:1"></div>
                        ...
                        ${key('F9', 'F9')} ${key('F10', 'F10')} ${key('F11', 'F11')} ${key('F12', 'F12')} <div class="key-placeholder" style="flex-grow:1"></div>
