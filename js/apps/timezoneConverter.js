...
let dateInput, timeInput, addTimezoneSelect, addTimezoneBtn, outputContainer, setCurrentTimeBtn, localTimezoneDisplay;
let targetTimezones = new Set(); // Используем Set для хранения уникальных часовых поясов
...
export function getHtml() {
    return `
        <div class="flex flex-col gap-6 p-2 md:p-4">
            ...
            <!-- Блок для добавления часовых поясов -->
            <div class="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg shadow-inner">
                 <h3 class="text-lg font-semibold mb-3 text-gray-800 dark:text-gray-200">Добавить часовой пояс</h3>
                <div class="flex items-end gap-4">
                    <div class="flex-grow">
                        <label for="add-timezone-select" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Выберите пояс</label>
                        <select id="add-timezone-select" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white dark:bg-gray-800"></select>
                    </div>
            ...
            <div id="output-timezones" class="mt-4 space-y-4">
...
export function init() {
    ...
    // Заполняем выпадающий список часовыми поясами
    populateTimezoneSelect();
    
    // Определяем и отображаем локальный часовой пояс пользователя
    const localTz = Intl.DateTimeFormat().resolvedOptions().timeZone;
    localTimezoneDisplay.textContent = localTz;
    
    // Добавляем по умолчанию пару популярных часовых поясов
    targetTimezones.add(localTz); // Добавляем пояс пользователя
    targetTimezones.add('UTC');
    targetTimezones.add('America/New_York');
    targetTimezones.add('Europe/London');

    // Устанавливаем текущее время при инициализации
    setCurrentTime();
    
    // Добавляем обработчики событий
    addEventListeners();
    
    // Первично отображаем конвертацию
    renderAllTimezones();
}
...
function populateTimezoneSelect() {
    ...
    timezones.forEach(tz => {
        ...
        option.value = tz;
        option.textContent = tz.replace(/_/g, ' '); // Заменяем подчеркивания на пробелы для читаемости
...
function renderAllTimezones() {
    ...
    if (!dateInput.value || !timeInput.value) {
        return; // Не делаем ничего, если дата или время не установлены
    }

    // Создаем объект Date на основе локального времени, введенного пользователем
    const sourceDate = new Date(`${dateInput.value}T${timeInput.value}`);

    // Сортируем часовые пояса для предсказуемого порядка
...
// Функция для получения смещения UTC в формате "+HH:MM"
