...
function drawWheel() {
    ...
    const R_outer = R - 5;
    const R_text_area = R * 0.35; // Толщина внешнего кольца для текста

    arc = options.length > 0 ? Math.PI / (options.length / 2) : 0;
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    
    // Рисуем цветные сектора на всю область
    ...
    // --- РЕШЕНИЕ: Рисуем полупрозрачное темное кольцо поверх секторов ---
    ctx.fillStyle = 'rgba(0, 0, 0, 0.4)';
    ctx.beginPath();
    ctx.arc(R, R, R_outer, 0, Math.PI * 2, false); // Внешняя граница
    ctx.arc(R, R, R_outer - R_text_area, 0, Math.PI * 2, true); // Внутренняя граница (дырка)
    ctx.fill();

    // Рисуем текст на этом кольце
    ctx.save();
    ...
    for (let i = 0; i < options.length; i++) {
        ...
        const textRadius = R_outer - (R_text_area / 2); // Центрируем текст внутри кольца
