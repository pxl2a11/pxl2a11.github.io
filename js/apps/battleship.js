...
export function init() {
    ...
    const getShipsConfig = () => JSON.parse(JSON.stringify([
        ...
        { id: 2, name: "Крейсер", size: 3, isPlaced: false },
        { id: 3, name: "Эсминец", size: 2, isPlaced: false },
        { id: 4, name: "Эсминец", size: 2, isPlaced: false },
        { id: 5, name: "Эсминец", size: 2, isPlaced: false },
        { id: 6, name: "Торпедный катер", size: 1, isPlaced: false },
        { id: 7, name: "Торпедный катер", size: 1, isPlaced: false },
        { id: 8, name: "Торпедный катер", size: 1, isPlaced: false },
        { id: 9, name: "Торпедный катер", size: 1, isPlaced: false }
    ]));
    ...
    const dom = {
        ...
        shipSelectionList: document.getElementById('ship-selection-list'),
        orientationCheckbox: document.getElementById('orientation-checkbox'),
        randomPlaceBtn: document.getElementById('random-place-btn'),
        startGameBtn: document.getElementById('start-game-btn'),
        resetBtn: document.getElementById('reset-btn'),
        gameStatus: document.getElementById('game-status'),
        playerGameGrid: document.getElementById('player-game-grid'),
        computerGameGrid: document.getElementById('computer-game-grid'),
    ...
    function renderShipSelection() {
        ...
        dom.startGameBtn.disabled = !allPlaced;
         if (allPlaced) {
            dom.shipSelectionList.innerHTML = '<p class="p-2 text-green-600 dark:text-green-400">Флот готов к бою!</p>';
        }
    }
    ...
    function handleSetupCellMouseOver(e) {
        ...
        for (let i = 0; i < selectedShip.size; i++) {
            ...
            if (previewX < GRID_SIZE && previewY < GRID_SIZE) {
                const cell = dom.playerSetupGrid.querySelector(`[data-x='${previewX}'][data-y='${previewY}']`);
    ...
    function handleSetupCellMouseOut() {
        dom.playerSetupGrid.querySelectorAll('.bs-cell').forEach(c => {
    ...
    function handleComputerGridClick(e) {
        ...
        isPlayerTurn = false;
        dom.gameStatus.textContent = hitShip ? "Попадание! Ход противника..." : "Промах. Ход противника...";
    ...
    function computerTurn() {
        ...
        isPlayerTurn = true;
        dom.gameStatus.textContent = "Ваш ход";
    }
    ...
    function checkAndMarkSunkShips(board, x, y) {
        ...
        const shipCells = [];
        board.forEach((row, r_idx) => row.forEach((cell, c_idx) => {
            if (cell.shipId === shipId) shipCells.push({x: c_idx, y: r_idx});
        }));
            ...
            shipCells.forEach(cell => {
                ...
                for (let dy = -1; dy <= 1; dy++) {
                    for (let dx = -1; dx <= 1; dx++) {
                        ...
                        if (checkX >= 0 && checkX < 10 && checkY >= 0 && checkY < 10 && !board[checkY][checkX].isHit) {
                            board[checkY][checkX].isHit = true;
                        }
                    }
                }
            });
    ...
    function checkWinCondition(board) {
        return getShipsConfig().every(ship => {
             const shipCells = [];
             board.forEach((row, r_idx) => row.forEach((cell, c_idx) => {
                if (cell.shipId === ship.id) shipCells.push(cell);
             }));
    ...
    function initGame() {
        ...
        dom.orientationCheckbox.checked = false;
    ...
    reportResult(x, y, isHit) {
        if (isHit) {
            ...
            if (this.currentHits.length >= 2) {
                ...
                if(isVertical) {
                    this.addToQueue(x, min - 1); this.addToQueue(x, max + 1);
                } else {
                    this.addToQueue(min - 1, y); this.addToQueue(max + 1, y);
                }
            } else {
                 this.addToQueue(x + 1, y); this.addToQueue(x - 1, y);
                 this.addToQueue(x, y + 1); this.addToQueue(x, y - 1);
            }
        }
    }
    reportSunk() {
