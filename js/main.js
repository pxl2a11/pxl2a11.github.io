...
async function renderSidebar(currentAppModule) {
    ...
    // --- 1. Подготовка данных для всех категорий ---
    const myApps = await getMyApps();
    ...
    sidebarList.innerHTML = ''; // Очищаем контейнер

    // --- 2. Создание HTML для каждой категории ---
    ...
    // --- 5. Обновление обработчика поиска ---
    const sidebarSearchHandler = () => {
        const searchTerm = searchInput.value.toLowerCase().trim();
        
        sidebarList.querySelectorAll('.sidebar-category').forEach(categoryContainer => {
            let matchesFoundInCategory = false;
            categoryContainer.querySelectorAll('.app-item').forEach(appItem => {
                const appName = appItem.dataset.name.toLowerCase();
                const moduleName = appItem.dataset.module;
                const metadata = appSearchMetadata[moduleName] || { keywords: [] };
                const searchCorpus = [appName, ...metadata.keywords].join(' ');
                
                const isVisible = searchTerm === '' || searchCorpus.includes(searchTerm);
                appItem.style.display = isVisible ? 'flex' : 'none';
                if (isVisible && searchTerm !== '') {
                    matchesFoundInCategory = true;
                }
            });

            // Если есть совпадения, раскрываем категорию. Если нет, сворачиваем.
            if (searchTerm !== '') {
                categoryContainer.classList.toggle('expanded', matchesFoundInCategory);
            }
        });
    };
