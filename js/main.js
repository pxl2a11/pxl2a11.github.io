...
function updateAuthStateUI(user) {
    ...
    if (user) {
        ...
        sidebarMyAppsButton?.classList.remove('hidden');
        ...
        sidebarMyAppsButton?.classList.add('hidden');
...
async function renderSidebar(currentAppModule) {
    ...
    const filterContainer = document.getElementById('sidebar-filter-container');
    ...
    const applySidebarFilter = async () => {
        ...
        if (currentActiveFilterInSidebar === 'my-apps') {
            ...
            appsToRenderModules = sortedApps.map(card => card.dataset.module);
        }

        sidebarList.innerHTML = '';
        appsToRenderModules.forEach(moduleName => {
